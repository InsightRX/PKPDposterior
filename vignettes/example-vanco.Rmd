...
installed using the `PKPDmodels` repository available [here](https://github.com/InsightRX/PKPDmodels). After cloning this repository,
install the `pkvancothomson` package using:

```{r vanco-pkg, eval=F}
model_from_api("models/pk_vanco_thomson.json5", to_package = TRUE, install_all = TRUE)
```
